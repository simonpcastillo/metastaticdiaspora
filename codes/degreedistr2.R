degreedistr2 <- function(web, plot.it=TRUE, pure.call=TRUE, silent=TRUE, level="both", ...){
    ...
    if (plot.it){
      plotfit <- function(data, fit, ...){
        plot(data$P ~ data$k, log="xy", pch=16, cex=1,xlab="Degree", ylab="", ...)
        abline(h=1, lty=2)
        for (i in 1:3){
      ...
      if (lower) plotfit(data=Plower, fit=fitl, lwd=2, cex.lab=1.5, main="lower", ...)
      if (higher) plotfit(data=Phigher, fit=fith, lwd=2, cex.lab=1.5, main="higher", ...)
    }
    
    modLow<- data.frame(k=sample(1000, min=1:max(Plower$k)))
    modLow$P=predict(fitl[[3]], newdata = k,type='response')
    plot(modLow$k, modLow$P)
    
    ggplot(data=NULL) +
      geom_point(data=Plower, aes(k,P), colour=viridis::viridis(1, direction = -1))+
      geom_line(aes(x=seq(1, max(Plower$k), by=0.1),y=predict(fitl[[3]],newdata=data.frame(k=seq(1, max(Plower$k), by=0.1)))),colour=viridis::viridis(1, direction = -1))+
      geom_point(data=Phigher, aes(k,P),colour=viridis::viridis(1, direction = 1))+
      geom_line(aes(x=seq(1, max(Phigher$k), by=0.1),y=predict(fith[[3]],newdata=data.frame(k=seq(1, max(Phigher$k), by=0.1)))),colour=viridis::viridis(1, direction = 1))+
      theme_minimal()+
      scale_y_log10()+
      scale_x_log10()
    ...
    res.out <- matrix(ncol=6, nrow=3)
    rownames(res.out) <- c("exponential", "power law", "truncated power law")
    colnames(res.out) <- c("Estimate", "Std. Error", "Pr(>|t|)", "R2", "AICc", "BIC")
    ...
    if (!only.na.low){ # catch the case that none of the regressions converges:
      resl2 <- t(sapply(fitl[indexl], function(mod) try(c(R2=cor(eval(mod$data)$P, predict(mod)), AICc=AICc(mod),BIC=BIC(mod)), silent=TRUE )))
      resl.out[indexl, 4:6] <- resl2
    ...
    if (!only.na.high){ # catch the case that none of the regressions converges:
      resh2 <- t(sapply(fith[indexh], function(mod) try(c(R2=cor(eval(mod$data)$P, predict(mod)), AICc=AICc(mod), BIC=BIC(mod)), silent=TRUE )))
      resh.out[indexh, 4:6] <- resh2
