'degreedistr3' <-
  function(web, plot.it=TRUE, pure.call=TRUE, silent=TRUE, level="both", cols.list = c('green', 'magenta', 'blue'), ...){
    require(MuMIn)
    ...
    if (plot.it){
      plotfit <- function(data, fit, ...){
        ...
        #abline(h=1, lty=2)
        for (i in 1:3){
          if (!inherits(fit[[i]], "try-error")) lines(seq(1, max(ddlower), by=0.1), 
                                                      predict(fit[[i]],
                                                              newdata=data.frame(k=seq(1, max(ddlower), by=0.1))), 
                                                      col=cols.list[i],...)
        }
        
      }
      if (pure.call & level == "both") par(mfrow=c(1,2), mar=c(5,5,4,1)) else par(mar=c(5,5,4,1))
      if (lower) plotfit(data=Plower, fit=fitl, lwd=2, cex=1.8, cex.lab=1.5, main="Source organs", ...)
      if (higher) plotfit(data=Phigher, fit=fith, lwd=2, cex=1.8, cex.lab=1.5, main="Acceptor organs", ...)
    }
    ...
    resl.out <- as.data.frame(resl.out)
    resh.out <- as.data.frame(resh.out)
    
    resl.out %>%
      mutate(`Pr(>|t|)` = format(`Pr(>|t|)` , scientific=TRUE),
             Estimate = round(Estimate, 3),
             `Std. Error` = round(`Std. Error`, 3),
             R2 = round(R2, 3),
             AICc = round(AICc, 3),
             BIC = round(BIC,3)) -> resl.out
    ...
    resh.out %>%
      mutate(`Pr(>|t|)` = format(`Pr(>|t|)` , scientific=TRUE),
             ...
             R2 = round(R2, 3),
             AICc = round(AICc, 3),
             BIC = round(BIC,3)) -> resh.out
    
    
    if (level == "both"){
      out <- list("Source organs' degree distribution fits"=resl.out, "Acceptor organs' degree distribution fits"=resh.out)
    } else {
      out <- if(lower) list("Source organs' degree distribution fits"=resl.out) else "Acceptor organs' degree distribution fits"=resh.out
    }
    return(out)
